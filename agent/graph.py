...
def planner_agent(state: dict) -> dict:
    ...
    resp = groq_model_llama_4_scout_17b.with_structured_output(Plan).invoke(planner_prompt(user_prompt))
    print(resp)
    return {"plan": resp}
...
def architect_agent(state: dict) -> dict:
    ...
    resp = groq_model_llama_4_scout_17b.with_structured_output(TaskPlan, method="function_calling").invoke(architect_prompt(plan))
    if resp is None:
